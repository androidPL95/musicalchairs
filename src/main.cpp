...
void JogoDasCadeiras::exibir_estado() {
    ...
    for (size_t i = 1; i <= this->cadeiras_.size(); i++) {
        std::cout << "[Cadeira " << i << "]: Ocupada por P" << this->cadeiras_[i-1].get_id_jogador() << std::endl;
    }
    std::cout << "Jogador P" << this->jogadores_eliminados_.back() << " não conseguiu uma cadeira e foi eliminado!"<< std::endl;
...
void Coordenador::iniciar_jogo(Coordenador *coordenador) {
    ...
    while (jogo_ativo) {
        ...
        if (coordenador->jogo_.get_num_jogadores() == 1) {
            ...
            std::cout << "🏆 Vencedor: Jogador P" << coordenador->jogo_.get_id_vencedor() << "! Parabéns! 🏆" << std::endl;
